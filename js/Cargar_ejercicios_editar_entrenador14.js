...
//Funcion para que el div de opciones cambie dependiendo del dia seleccionado
function seleccionarDia(dia) {
    console.log('Día seleccionado:', dia);

    // Mostrar solo el contenido correspondiente a ese día
    const opcionesContainer = document.querySelector('.opciones');
    opcionesContainer.style.display = 'block'; // Aseguramos que las opciones estén visibles

    // Limpiar la sección de ejercicios
    const exerciseList = document.querySelector('.exercise-list');
    exerciseList.innerHTML = '';

    // Aquí puedes cargar los ejercicios para ese día desde la base de datos
    // Llamada para obtener ejercicios de la base de datos (por ejemplo, usando AJAX o fetch)
    fetch(`api/GET/obtener_ejercicios.php?dia=${dia}`)
        .then(response => response.json())
        .then(data => {
            if (data.success && data.data) {
                // Aquí, mostramos los ejercicios para ese día
                data.data.forEach(ejercicio => {
                    const ejercicioElement = document.createElement('div');
                    ejercicioElement.classList.add('ejercicio');
                    ejercicioElement.innerHTML = `
                        <p>${ejercicio.nombre} - Reps: ${ejercicio.repeticiones}</p>
                    `;
